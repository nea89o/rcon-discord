    ...
    public void execute(GuildMessageReceivedEvent event, String[] args) {
        ...
        if (users.size() < 1) {
            Util.sendEmbed(event.getChannel(), "Syntax Error",
                    "Mention one or more users to unban", Color.RED, event.getAuthor());
            return;
        }
        GuildController controller = guild.getController();
        Role bannedRole = guild.getRoleById(Config.bannedRole);
        users.stream()
                .map(user -> controller.removeSingleRoleFromMember(user, bannedRole))
                .forEach(RestAction::queue);
        Util.sendEmbed(event.getChannel(), "Unbaned " + users.size() + " users", "Unbanned " +
                users.stream()
